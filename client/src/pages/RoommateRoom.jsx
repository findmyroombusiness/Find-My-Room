...
const RoomFormPC = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let res, data;
      const roommateRoomId = editMode && listing && listing._id ? listing._id : null;
      const apiBase = import.meta.env.VITE_API_BASE_URL;
      if (editMode) {
        if (!roommateRoomId) {
          setError("Missing Roommate Room ID. Cannot update.");
          alert("Missing Roommate Room ID. Cannot update.");
          setLoading(false);
          return;
        }
        const putUrl = `${apiBase}/api/roommaterooms/${roommateRoomId}`;
        ...
        const postUrl = `${apiBase}/api/roommaterooms`;
      ...
      if (!res.ok) {
        if (res.status === 404) {
          setError("Roommate Room not found or deleted.");
          alert("Roommate Room not found or deleted.");
        } else {
...
const RoommateRoomWithFooter = () => (
  <>
    <Room />
    <Footer />
  </>
);
export default RoommateRoomWithFooter;
