...
const HostelList = () => {
  ...
  useEffect(() => {
    ...
    apiFetch(`${import.meta.env.VITE_API_BASE_URL}/api/hostels?page=1&limit=3`)
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (
        ...
        setLoading(true);
        apiFetch(
          `${import.meta.env.VITE_API_BASE_URL}/api/hostels?page=${page}&limit=3`
        )
          .then((res) => res.json())
          .then((data) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6 md:mb-0">
              {sortedListings.map((listing) => (
                <div key={listing._id} className="relative">
                  <Link to={`/hosteldetail/${listing._id}`}>
                    <div className="bg-white rounded-2xl shadow-md overflow-hidden hover:scale-105 transition-transform cursor-pointer group">
                      ...
                      <div className="p-4">
                        <h2 className="text-xl font-bold text-[rgb(77,95,171)]">
                          {listing.rent} / month
                        </h2>
...
export default HostelListWithFooter;
