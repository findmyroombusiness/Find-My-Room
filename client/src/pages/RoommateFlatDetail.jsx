    ...
    const fetchFlat = async () => {
      ...
      try {
        const res = await apiFetch(
          `${import.meta.env.VITE_API_BASE_URL}/api/roommateflats/${id}`
        );
      ...
      <div className="bg-gray-100 md:py-7 px-4 pb-4 pt-20 md:pt-10">
        <div className="max-w-6xl mx-auto bg-white rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="p-6 md:p-10">
            <h1 className="text-2xl font-bold text-[rgb(77,95,171)] mb-4">Flat Details</h1>
            ...
            {flat.user && (
              <div className="mt-6 text-sm text-gray-500">Posted by: {flat.user.name || flat.user.email}</div>
            )}
            <div className="mt-2 text-sm text-gray-400">Posted on: {new Date(flat.createdAt).toLocaleDateString()}</div>
          </div>
        </div>
      </div>
    </div>
...
const RoommateFlatDetailWithFooter = () => (
  <>
    <RoommateFlatDetail />
    <Footer />
  </>
);
export default RoommateFlatDetailWithFooter;
