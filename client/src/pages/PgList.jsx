...
const BACKEND_URL = import.meta.env.VITE_API_BASE_URL || 'https://find-my-room-backend.onrender.com';
...
const PGList = () => {
  ...
  useEffect(() => {
    ...
    apiFetch(`${BACKEND_URL}/api/pgs?page=1&limit=3`)
      .then(res => res.json())
      .then(data => {
    ...
    if (token) {
      apiFetch(`${BACKEND_URL}/api/favourites`, {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (
        ...
        setLoading(true);
        apiFetch(`${BACKEND_URL}/api/pgs?page=${page}&limit=3`)
          .then(res => res.json())
          .then(data => {
  ...
  const toggleFavourite = async (id) => {
    ...
    try {
      ...
      const res = await apiFetch(`${BACKEND_URL}/api/favourites`, {
        headers: { Authorization: `Bearer ${token}` },
      });
