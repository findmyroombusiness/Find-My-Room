...
const ImageUpload = ({ images, setImages, onlyCloudinary }) => {
  React.useEffect(() => {
    // Revoke any created object URLs when the component unmounts to avoid
    // revoking while the component is still rendering previews.
    return () => {
      images.forEach(img => {
    ...
    // Intentionally omit images from deps so we only run on unmount.
...
function FlatForm() {
  ...
  const [form, setForm] = useState(
    editMode && listing
      ? {
          ...
          bhk: listing.bhk ? String(listing.bhk) : "",
          ...
          food: listing.food || "",
          washrooms: listing.washrooms ? String(listing.washrooms) : "",
  ...
  // (no change, keep Cloudinary upload as is)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editMode && listing && listing._id) {
        const putUrl = `${import.meta.env.VITE_API_BASE_URL}/api/flats/${listing._id}`;
        ...
        res = await apiFetch(`${import.meta.env.VITE_API_BASE_URL}/api/flats`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
...
const FlatFormMobile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const token = localStorage.getItem("token");
      let res, data;
      const flatId = editMode && listing && listing._id ? listing._id : null;
      ...
      if (editMode) {
        ...
        const postUrl = `${apiBase}/api/flats`;
      ...
      if (!res.ok) {
        if (res.status === 404) {
          setError("Flat not found or deleted.");
          alert("Flat not found or deleted.");
        } else {
...
const FlatWithFooter = () => (
  <>
    <Flat />
    <Footer />
  </>
);
export default FlatWithFooter;
