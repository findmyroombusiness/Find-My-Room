...
const FlatList = () => {
  ...
  useEffect(() => {
    ...
    // Fetch first page of flats
    apiFetch(`${import.meta.env.VITE_API_BASE_URL}/api/flats?page=1&limit=3`)
      .then(res => res.json())
      .then(data => {
        if (!data || data.error) throw new Error(data.message || "Failed to fetch flats");
        setAllListings(data.listings || data); // support both array and {listings:[], total: n}
    ...
    // Fetch favourites for first page
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (
        ...
        setLoading(true);
        apiFetch(`${import.meta.env.VITE_API_BASE_URL}/api/flats?page=${page}&limit=3`)
          .then(res => res.json())
          .then(data => {
